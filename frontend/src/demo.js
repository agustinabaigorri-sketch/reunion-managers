...
export const demoApi = {
  ...
  okrMyPlan: (anio) => { ensureOkr(); const a = me().area_id; const yr = Number(anio) || 2026; const objs = store.okrAOs.filter((x) => x.area_id === a && x.anio === yr).map((x) => ({ ...x, colab_areas: x.colab_areas || [], colabs: (store.okrColabs || []).filter((c) => c.area_objective_id === x.id), metas: (store.okrMetas || []).filter((m) => m.area_objective_id === x.id) })); return wait({ anio: yr, area_id: a, objectives: objs }); },
  ...
  okrMetaAdd: (d) => { ensureOkr(); store.okrMetas = store.okrMetas || []; store.okrMetas.push({ id: iid(), area_objective_id: Number(d.area_objective_id), titulo: d.titulo || '', hecho: false, avance: 0, orden: store.okrMetas.length }); persist(); return wait({ ok: true }); },
